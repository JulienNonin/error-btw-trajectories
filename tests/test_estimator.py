...
/2_EmptyAcquiredTrajectory.txt", False)[1:]
    ...
    reference = estimator.Trajectory(
        [estimator.Point(x, y) for x, y in reference_coord])
    acquired = estimator.Trajectory(
        [estimator.Point(x, y) for x, y in acquired_coord])
...
Trajectory([Point(1.0, 2.0), Point(3.0, 2.0)]), 1.0, 0.001]
    ...
    for filename in sorted(test_file_names)[samples]:
        tests_sample.append(fetch_data(
